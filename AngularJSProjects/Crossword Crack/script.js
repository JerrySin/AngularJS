...
(function () {
    ...
    angular
        .module('jm.cw')
        .controller('CrosswordController', CrosswordController);

    CrosswordController.$inject = ['$scope', '$http', '$log', 'xWord'];

    function CrosswordController($scope, $http, $log, xWord) {
        //var letters = 
        activate();
        ...
        function activate() {
            //$http.get('letters.js').then(onComplete, onError);
            xWord.getWordArray().then(onComplete);
            //$scope.letters = xWord.dataArray;
        };

        $scope.showStatistics = function (currentChar) {
            $scope.count = globalCount(currentChar);
        };

        var globalCount = function (charValue) {
            var gCount = 0;
            angular.forEach($scope.letters, function (item) {
                if (item.char == charValue) {
                    gCount++;
                }
            });
            return gCount;
